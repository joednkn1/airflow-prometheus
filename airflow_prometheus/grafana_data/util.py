...
def get_df_col(col):
    if "node_graph_" in col:
        col = col.replace("node_graph_", "")
        return dict(
            text=f"arc__{col}",
            color=dict(
                mode="fixed",
...
def dataframe_to_json_table(target, df):
    ...
    if isinstance(df, pd.DataFrame):
        ...
        response.append({'type': 'table',
                         'columns': df.columns.map(get_df_col).tolist(),
        ...
        for frame in df:
            response.append({**frame[0],
                             'type': 'table',
                             'columns': frame[1].columns.map(get_df_col).tolist(),
                             'rows': frame[1].where(pd.notnull(frame[1]), None).values.tolist()})
    else:
